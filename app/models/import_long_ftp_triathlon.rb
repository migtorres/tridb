class ImportLongFtpTriathlon < ActiveRecord::Base
  ...
  def self.import_from_url(url, import_results_from_url_id)
    ...
    debugger
    ImportLongFtpTriathlon.long_ftp_triathlon(@page_number,@reader)
    end
  ...
  def self.get_last_results(result, split_array, break_point)
    if ["DQ","DNF", "DNS"].include? result[:pen]
      ...
      if split_array[break_point+2..-1].count == 6 || result[:place] == 1
        ...
        result[:transition_one_position] = split_array[break_point+1..-1][1].to_i
        result[:transition_one_time] = split_array[break_point+1..-1][2]
        result[:cycling_position] = split_array[break_point+1..-1][3].to_i
        result[:cycling_time] = split_array[break_point+1..-1][4]
        result[:transition_two_position] = split_array[break_point+1..-1][5].to_i
        result[:transition_two_time] = split_array[break_point+1..-1][6]
        result[:running_position] = split_array[break_point+1..-1][7].to_i
        result[:running_time] = split_array[break_point+1..-1][8]
...
=end
  ...
  def self.long_ftp_triathlon(page_number,reader)
    #@import_results_from_url_id = import_results_from_url_id
    page = 1	
